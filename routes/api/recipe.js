                ...
                Ingredient.destroy({ where: { ingredientable_id: req.params.id } })
                    ...
                    .then(response => {
                        ...
                        if (!newRecipe.ingredients || newRecipe.ingredients.length == 0) {
                            return Recipe.findById(req.params.id, { include: [modelOptions.ingredients] }).then(recipe => res.json(recipe));
                        }
                        ...
                        Ingredient.bulkCreate(newIngredients, { returning: true }).then(createdIngredients => {
                            ...
                            // Return the newly created recipe with the ingredients
                            Recipe.findById(req.params.id, { include: [modelOptions.ingredients] }).then(recipe => res.json(recipe));
                        });
                    })
            })
            .catch(err => res.status(404).json(err))
